...
public class StockUpdateQueue {
    ...
    @Autowired
    private OfflineMessageStorageMananger offlineMessageStorageMananger;
    ...
    public void add(StockUpdateMessage stockUpdateMessage){
        ...
        if(queue.size()==0){
            ...
            new Thread(new OfflineResumeThread(offlineMessageStorageMananger,stockMessageManager,stockUpdateQueue)).start();
        }
        queue.add(stockUpdateMessage);
    }
